...
class PatternGenerator:
    ...
    def zig_zag_curve(self, step=10, stride=0.2, orientation="vertical"):
        ...
        # 3. Computing the number of lines
        target_strides = np.arange(stride_start, stride_end, stride)

        # Ensure that the last border is covered
        # If the last line is not exactly on the outer border, we add an extra line on this border
        if target_strides[-1] < (stride_end - 1e-5):
            target_strides = np.append(target_strides, stride_end)
        else:
            target_strides[-1] = stride_end  # Make sure it's precise
        ...
        for i in range(num_lines):
            # Direction: 1 = up or right ; -1 = down or left

            direction = 1 if i % 2 == 0 else -1

            # --- A. STRAIGHT LINE---
            # If we go to the MAX (+1) we stop at (MAX + radius) to leave space for the curve
            # If we go to the MIN (-1) we stop at (MIN + radius)

            # Special case: Start of the trajectory (i=0, start) begins at the border
            if i == 0 and direction == 1:
                p_start = sweep_min
            else:
                p_start = sweep_min + R if direction == 1 else sweep_max - R

            # Special case: end of straight line
            # If it's the last line we go all the way to the end
            # If not we stop before the border to leave space for the curve
            if i == num_lines - 1:
                p_end = sweep_max if direction == 1 else sweep_min
            else:
                p_end = sweep_max - R if direction == 1 else sweep_min + R

            # Generating waypoints for the line
            ...
            # --- B. TURN-AROUND CURVE (intern) ---
            if i < num_lines - 1:
                # Generating a  180 degrees curve (pi rads)
                # We remove the first point (t=0) to avoid a duplicate with the end of the straight line
                ...
                # We start from current_stride_pos and go to current_stride_pos + stride
                ...
                # The curve must touch the border but without going further
                # If the direction is +1 (up) we must curve towards the up then come back down
                # The center of rotation is at (p_end)
                # Formula : center + R * sin(t) * direction

                # Pivot point (end of last line)
                pivot = coords_line[-1, axis_sweep]
                coords_arc[:, axis_sweep] = pivot + R * np.sin(t_arc) * direction

                trajectory_chunks.append(coords_arc)

                # Update the stride position
                current_stride_pos += stride

        # 4. Assemble all trajectories
        full_matrix = np.vstack(trajectory_chunks)

        # Conversion en liste de np.array([x,y,z])
